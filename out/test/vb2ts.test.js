...
const vb2ts_1 = require("../src/vb2ts");
...
suite("vb2ts Tests", () => {
    ...
    for (const p of testPairs) {
        for (const input of p.inputs) {
            test("Auto properties should get converted correctly", () => {
                assert.equal(vb2ts_1.default(input), p.output, input);
            });
        }
    }
    ...
    for (let vbFileName of vbFileNames) {
        ...
        test(vbFileName, () => {
            const actual = vb2ts_1.default(vbDeclaration);
...
//# sourceMappingURL=vb2ts.test.js.map